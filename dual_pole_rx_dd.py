...
MIMO XPIC: Hybrid CMA → Decision-Directed equalizer.
  - CMA (Constant Modulus Algorithm) runs for a configurable warmup
    period to achieve coarse convergence.
  - After warmup, switches to Decision-Directed (DD) mode using
    constellation-matched hard slicers for QPSK (ch0) and BPSK (ch1).
  - DD has much lower misadjustment noise than CMA because the error
    signal is based on known constellation geometry, not a blind
    modulus cost.
  - Includes tap leakage (pulls toward identity when signal is absent)
    and separate step sizes for direct vs cross-pol taps.
...
# ─── XPIC constants ────────────────────────────────────
# QPSK constellation points (normalized, Gray-coded)
...
##################################################
# NUMBA-Compiled Hybrid CMA → DD MIMO XPIC
##################################################

@nb.jit(nopython=True, cache=True)
def _qpsk_slice(y):
    """Hard slicer for QPSK — nearest constellation point."""
    # Branchless: sign of real/imag picks the quadrant
    INV_SQRT2 = 0.7071067811865476
    re = INV_SQRT2 if y.real >= 0 else -INV_SQRT2
    im = INV_SQRT2 if y.imag >= 0 else -INV_SQRT2
    return re + 1j * im


@nb.jit(nopython=True, cache=True)
def _bpsk_slice(y):
    """Hard slicer for BPSK — nearest constellation point."""
    re = 1.0 if y.real >= 0 else -1.0
    return re + 0j


@nb.jit(nopython=True, fastmath=True, nogil=True, cache=True)
def numba_cma_dd_xpic(in0, in1, out0, out1,
                       mu_direct, mu_cross, rho, leak,
                       w00, w01, w10, w11,
                       sample_count, warmup_samples,
                       err_acc0, err_acc1, err_count):
    """
    Hybrid CMA → Decision-Directed 2×2 MIMO XPIC equalizer.

    Phase 1 (sample_count < warmup_samples): CMA
        Error = y * (|y|² - R²)  where R² = 1.0 for both constellations.
        Blind convergence — works without knowing the constellation but
        has high steady-state misadjustment.

    Phase 2 (sample_count >= warmup_samples): Decision-Directed
        Error = y - slice(y)  where slice() snaps to nearest constellation
        point. Much lower misadjustment because the error signal tracks
        the actual symbol, not just the modulus.

    Both phases use:
      - Separate step sizes for direct taps (w00,w11) vs cross taps (w01,w10)
      - Cross-pol decorrelation penalty (rho)
      - Tap leakage toward identity matrix (leak)

    Parameters
    ----------
    in0, in1       : input sample arrays (complex64 from PFB)
    out0, out1     : output sample arrays
    mu_direct      : step size for direct-path taps (w00, w11)
    mu_cross       : step size for cross-path taps (w01, w10) — smaller
    rho            : cross-pol decorrelation strength
    leak           : leakage coefficient pulling taps toward identity
    w00..w11       : current tap values (complex128 for precision)
    sample_count   : running count of samples processed so far
    warmup_samples : number of samples to run CMA before switching to DD
    err_acc0/1     : running error magnitude accumulators (for diagnostics)
    err_count      : running count for error averaging

    Returns
    -------
    Updated w00, w01, w10, w11, sample_count, err_acc0, err_acc1, err_count
    """
    n = in0.shape[0]
    R2 = 1.0  # target modulus squared (same for QPSK and BPSK at unity power)
    ...
    for i in range(n):
        ...
        # Instead of a hard reset, smoothly shrink taps toward identity
        # when output power is excessive. This avoids the discontinuity
        # of the old hard reset.
        ...
        # ── Compute error based on current mode ──
        if sample_count < warmup_samples:
            # === CMA MODE ===
            # Blind — uses modulus distance from target radius
            err0 = y0 * (mag_sq0 - R2)
            err1 = y1 * (mag_sq1 - R2)
        else:
            # === DECISION-DIRECTED MODE ===
            # Constellation-aware — error is distance to nearest symbol
            d0 = _qpsk_slice(y0)   # ch0 is QPSK
            d1 = _bpsk_slice(y1)   # ch1 is BPSK
            err0 = y0 - d0
            err1 = y1 - d1

        # ── Cross-pol decorrelation penalty ──
        # Encourages the equalizer to minimize correlation between
        # its two outputs, which is the whole point of XPIC.
        cross = y0 * y1.conjugate()
        err0 = err0 + rho * y1 * cross.conjugate()
        err1 = err1 + rho * y0 * cross

        # ── Accumulate error magnitude for diagnostics ──
        err_acc0 += abs(err0)
        err_acc1 += abs(err1)
        err_count += 1

        # ── Tap update with separate step sizes ──
        x0c = x0.conjugate()
        x1c = x1.conjugate()

        w00 -= mu_direct * err0 * x0c    # direct path
        w01 -= mu_cross  * err0 * x1c    # cross path
        w10 -= mu_cross  * err1 * x0c    # cross path
        w11 -= mu_direct * err1 * x1c    # direct path

        # ── Tap leakage toward identity ──
        # Prevents drift when one channel carries no signal (e.g.,
        # BPSK Tx off). Without this, cross taps fit to noise.
    ...
    return w00, w01, w10, w11, sample_count, err_acc0, err_acc1, err_count
...
class mimo_xpic_2x2(gr.sync_block):
    ...
    Modes:
      CMA   (samples 0..warmup-1): blind convergence, works cold-start
      DD    (samples warmup..):     constellation-matched, low misadjustment

    Tunable parameters (via constructor):
      mu_direct      : LMS step size for diagonal taps (default 1e-4)
      mu_cross       : LMS step size for off-diagonal taps (default 3e-5)
      rho            : cross-pol decorrelation penalty (default 0.05)
      leak           : tap leakage toward identity (default 1e-6)
      warmup_samples : CMA warmup duration (default 50000)
    """

    def __init__(self, mu_direct=1e-4, mu_cross=3e-5,
                 rho=0.05, leak=1e-6, warmup_samples=50000):
        gr.sync_block.__init__(self,
                               name="MIMO XPIC 2x2 (CMA→DD)",
        ...
        # Tap state — complex128 for numerical precision during adaptation
        self.w00 = 1.0 + 0j
        self.w01 = 0.0 + 0j
        self.w10 = 0.0 + 0j
        self.w11 = 1.0 + 0j
    ...
    def work(self, input_items, output_items):
        (self.w00, self.w01, self.w10, self.w11,
         self.sample_count,
         self.err_acc0, self.err_acc1, self.err_count) = numba_cma_dd_xpic(
        ...
        # Announce mode switch once
        if not self._announced_dd and self.sample_count >= self.warmup_samples:
            self._announced_dd = True
            print(f"[XPIC] Switched to Decision-Directed mode at "
                  f"sample {self.sample_count}")
            self._print_taps()

        return len(input_items[0])

    def _print_taps(self):
        print(f"[XPIC] Taps: w00={self.w00:.4f}  w01={self.w01:.4f}  "
              f"w10={self.w10:.4f}  w11={self.w11:.4f}")
...
class dual_pole_rx(gr.top_block, Qt.QWidget):
    def __init__(self):
        ...
        ##################################################
        # MIMO XPIC — Hybrid CMA → DD
        #
        # mu_direct=1e-4    : direct tap step size (same as before)
        # mu_cross=3e-5     : cross tap step size (conservative)
        # rho=0.05          : cross-pol decorrelation
        # leak=1e-6         : tap leakage toward identity
        # warmup=50000      : ~50ms at 1 MS/s, then switch to DD
        ##################################################
        ...
        print("[INIT] Flowgraph built (XPIC mode: CMA→DD). Starting...")
    ...
    def _tick(self):
        ...
        # ── Cross-pol isolation + error ──
        mode = self.mimo.get_mode()
        ...
        self.xpic_status.setText(
            f"XPIC [{mode}]  XPI: {iso0:.1f}/{iso1:.1f} dB  |  "
...
def main():
    ...
    print("[RUN] Flowgraph running (XPIC: CMA→DD hybrid).")
    print("[RUN] Watch terminal for '[XPIC] Switched to Decision-Directed'")
    print("[RUN] XPIC status bar shows tap magnitudes and cross-pol isolation.")
    print("[RUN] If CRC-ok stays 0 after DD switch, try increasing warmup_samples.")
