...
ML_SEQ_LEN   = 128
ML_INPUT_DIM = 2
ML_MODEL_DIR = os.path.join(os.path.expanduser("~"), "OneDrive", "Desktop")
...
##################################################
# RadioLSTM — modulation classifier (must match train.py)
##################################################

class RadioLSTM(tnn.Module):
    def __init__(self, input_dim=ML_INPUT_DIM, num_classes=11):
        super().__init__()
        self.lstm1      = tnn.LSTM(input_dim, 128, batch_first=True, bidirectional=True)
        self.dropout1   = tnn.Dropout(0.3)
        self.lstm2      = tnn.LSTM(256, 128, batch_first=True)
        self.batchnorm  = tnn.BatchNorm1d(256)
        self.fc         = tnn.Linear(256, 128)
        self.dropout2   = tnn.Dropout(0.2)
        self.classifier = tnn.Linear(128, num_classes)
    ...
    def forward(self, x):
        ...
        x      = self.batchnorm(x)
        return self.classifier(self.dropout2(tnnF.relu(self.fc(x))))


##################################################
# Modulation Classifier — pass-through block
# Periodically grabs 128 IQ samples from the
# stream, runs RadioLSTM inference, and stores
# the predicted modulation + confidence.
##################################################

class ModulationClassifierBlock(gr.sync_block):
    def __init__(self, model, scaler, classes, device, channel='',
                 classify_interval=50000):
        gr.sync_block.__init__(self,
    ...
    def work(self, input_items, output_items):
        ...
        if self.sample_count >= self.classify_interval:
            self.sample_count = 0
            seg = input_items[0][:min(ML_SEQ_LEN, n)].copy()
            self._classify(seg)
    ...
    def _classify(self, iq_complex):
        try:
            if len(iq_complex) >= ML_SEQ_LEN:
                iq_complex = iq_complex[:ML_SEQ_LEN]
            else:
                iq_complex = np.pad(iq_complex,
                                    (0, ML_SEQ_LEN - len(iq_complex)))

            iq = np.stack([iq_complex.real, iq_complex.imag],
                          axis=1).astype(np.float32)
            iq = self.scaler.transform(iq)
            x = torch.tensor(iq, dtype=torch.float32).unsqueeze(0).to(self.device)

            with torch.no_grad():
                probs = torch.softmax(self.ml_model(x), dim=1)
                idx   = probs.argmax().item()
                conf  = probs[0, idx].item()

            self.prediction = self.classes[idx]
...
class dual_pole_rx(gr.top_block, Qt.QWidget):
    def __init__(self):
        ...
        ##################################################
        # Modulation Classifier — RadioLSTM after XPIC
        ##################################################

        ml_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        ml_classes = joblib.load(os.path.join(ML_MODEL_DIR,
                                              "class_labels (1).joblib"))
        ml_scaler  = joblib.load(os.path.join(ML_MODEL_DIR,
                                              "embed_scaler.joblib"))
        ml_model   = RadioLSTM(input_dim=ML_INPUT_DIM,
                               num_classes=len(ml_classes))
        ml_model.load_state_dict(
            torch.load(os.path.join(ML_MODEL_DIR, "model (1).pt"),
                       map_location=ml_device)
        )
        ml_model.to(ml_device)
        ml_model.eval()

        print(f"[ML] RadioLSTM loaded — {len(ml_classes)} classes on {ml_device}")

        self.mod_clf_0 = ModulationClassifierBlock(
            ml_model, ml_scaler, ml_classes, ml_device,
            channel='QPSK', classify_interval=50000)
        self.mod_clf_1 = ModulationClassifierBlock(
            ml_model, ml_scaler, ml_classes, ml_device,
            channel='BPSK', classify_interval=50000)
    ...
    def _tick(self):
        ...
        self.mod_status.setText(
            f"Modulation  CH0: {pred0} ({conf0:.0%})  |  "
            f"CH1: {pred1} ({conf1:.0%})")
