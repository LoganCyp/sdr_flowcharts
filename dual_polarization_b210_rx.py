...
class ModulationClassifierBlock(gr.sync_block):
    def __init__(self, model, labels, device, channel='',
                 classify_interval=10000, slice_len=1024):
        gr.sync_block.__init__(self,
...
def load_rfml(model_dir):
    ...
    """
    config_path = os.path.join(model_dir, "config_dp.json")
    weights_path = os.path.join(model_dir, "iq_classifier_dp.pth")
...
class dual_pole_rx(gr.top_block, Qt.QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Dual-Polarization Image Receiver")
        self.resize(950, 620)
       ...
       # === CH0: QPSK ===
        ...
        self.connect((self.agc_0, 0), (self.fll_0, 0))    # AGC -> FLL
        self.connect((self.fll_0, 0), (self.pfb_0, 0))    # FLL -> PFB (Timing)
        self.connect((self.pfb_0, 0), (self.mimo, 0))     # PFB -> MIMO
        self.connect((self.mimo, 0),  (self.costas_0, 0)) # MIMO -> Costas
        
        # MOD CLF moved here: Receiver is now phase-locked
        self.connect((self.costas_0, 0), (self.mod_clf_0, 0)) 
        
        # Continue to decoding chain
        self.connect((self.mod_clf_0, 0), (self.dec_q, 0))
        ...
        self.connect((self.agc_1, 0), (self.fll_1, 0))    # AGC -> FLL
        self.connect((self.fll_1, 0), (self.pfb_1, 0))    # FLL -> PFB
        self.connect((self.pfb_1, 0), (self.mimo, 1))     # PFB -> MIMO
        self.connect((self.mimo, 1),  (self.costas_1, 0)) # MIMO -> Costas

        # MOD CLF moved here
        self.connect((self.costas_1, 0), (self.mod_clf_1, 0))

        # Continue to decoding chain
        self.connect((self.mod_clf_1, 0), (self.dec_b, 0))
